...
export default class HomePage {
  ...
  createStoryHTML(story, index) {
    ...
    return `
      ...
      <div class="story-actions">
        <button class="action-btn like-btn">❤️ Suka</button>
        <button class="action-btn comment-btn">💬 Komentar</button>
        <button class="action-btn share-btn">🔗 Bagikan</button>
      </div>
    `;
  }
  ...
  initializeMap(index, lat, lon, name, description) {
    try {
      ...
      if (!mapElement || this.mapInstances[mapId]) {
        // Elemen tidak ada atau peta sudah diinisialisasi
        return;
      }

      const map = L.map(mapId).setView([lat, lon], 13);
      this.mapInstances[mapId] = map; // Simpan instance peta
  ...
  disconnectedCallback() {
    // Bersihkan instance peta saat komponen di-unmount (jika diperlukan)
